...
def draw_use_case_diagram():
    ...
    # Vẽ các tác nhân với kích thước nhỏ hơn
    for actor, pos in actor_positions.items():
        # Vẽ hình người đơn giản
        circle = plt.Circle(pos, 0.12, facecolor='white', edgecolor='black')
        line = plt.Line2D([pos[0], pos[0]], [pos[1]-0.12, pos[1]-0.4], color='black')
        arm1 = plt.Line2D([pos[0]-0.12, pos[0]+0.12], [pos[1]-0.25, pos[1]-0.25], color='black')
        leg1 = plt.Line2D([pos[0], pos[0]-0.08], [pos[1]-0.4, pos[1]-0.6], color='black')
        leg2 = plt.Line2D([pos[0], pos[0]+0.08], [pos[1]-0.4, pos[1]-0.6], color='black')
        ...
        # Thêm tên actor
        ax.text(pos[0], pos[1]-0.8, actor, ha='center', fontsize=10, fontweight='bold')
    ...
    # Cột giữa - Đã loại bỏ "Đặt Bàn"
    middle_column = {
        'Đăng Nhập/Đăng Ký': (5, 8.5),
        'Thanh Toán': (5, 7.5),
        'Quản Lý Địa Chỉ': (5, 6.5),
        'Đổi Mật Khẩu': (5, 5.5),
        'Cập Nhật Vị Trí': (5, 4.5),
        'Xem Báo Cáo': (5, 3.5),
        'Quản Lý Danh Mục': (5, 2.5)
    }
    ...
    # Vẽ các use case với kích thước nhỏ hơn
    for use_case, pos in use_cases.items():
        ellipse = Ellipse(pos, 1.4, 0.5, fill=True, color='lightblue', alpha=0.5)
        ax.add_patch(ellipse)
        ax.text(pos[0], pos[1], use_case, ha='center', va='center', fontsize=8)
    ...
    # Vẽ kết nối với màu đậm và rõ ràng hơn
    for actor, use_case in connections:
        ...
        if actor == 'Khách Hàng':
            ...
            color = '#00BFA5'  # Xanh lá đậm
            rad = 0.15
        elif actor == 'Chủ Nhà Hàng':
            color = '#7C4DFF'  # Tím đậm
            rad = -0.1
        else:  # Quản Trị Viên
        ...
        arrow = FancyArrowPatch(actor_pos, use_case_pos, connectionstyle=f"arc3,rad={rad}", 
                              arrowstyle="->", color=color, lw=1.2, alpha=0.8, shrinkA=20, shrinkB=10)
        ax.add_patch(arrow)
    ...
    for use_case1, use_case2, rel_type in relationships:
        ...
        arrow = FancyArrowPatch(use_case1_pos, use_case2_pos, connectionstyle="arc3,rad=-0.3", 
                              arrowstyle="-|>", linestyle=style, color='#1976D2', lw=1.2, shrinkA=10, shrinkB=10)
        ax.add_patch(arrow)
        ...
        label = ax.text(mid_x + offset_x, mid_y + offset_y, f'<<{rel_type}>>', 
                 fontsize=8, ha='center', va='center', color='#1976D2',
                 bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=1))
    
    # Thêm chú thích
    legend_elements = [
        plt.Line2D([0], [0], color='#FF5252', lw=2, label='Khách Hàng'),
        plt.Line2D([0], [0], color='#00BFA5', lw=2, label='Shipper'),
        plt.Line2D([0], [0], color='#7C4DFF', lw=2, label='Chủ Nhà Hàng'),
        plt.Line2D([0], [0], color='#FFC107', lw=2, label='Quản Trị Viên'),
        plt.Line2D([0], [0], color='#1976D2', lw=2, linestyle='solid', label='<<bao gồm>>'),
        plt.Line2D([0], [0], color='#1976D2', lw=2, linestyle='dashed', label='<<mở rộng>>')
    ]
    ...
    plt.ylim(0, 9)
    plt.axis('off')
    plt.title('Biểu Đồ Use Case FoodZone', fontsize=18, fontweight='bold')
    plt.tight_layout()
    plt.savefig('Draw/Usecase_Diagram/usecase_diagram.png', dpi=300, bbox_inches='tight')
    plt.show()
